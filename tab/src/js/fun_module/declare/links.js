const $linksEles = (function () {
  ...
  const renderLinks = (
    ...
    if (render_arr.length < 15) {
      render_arr.push(
        `<div class="links-td td-jia"><a><div class="add-link-icon"><span class="iconfont icon-jia" id="iconjia"></span></div></a></div>`
      );
    }
    ...
    if (linksinfo.length >= pageSize) {
      ...
      for (let i = 0; i < indicatorDotsNum; i++) {
        let dotHtmlStr = `<div class="indicator-dot ${
          parseInt(pageNum) === i + 1 ? "indicator-dot-active" : ""
        }"  data-ind="${i + 1}"></div>`;
